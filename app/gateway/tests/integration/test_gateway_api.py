...
def test_proxy_conversations_without_jwt_returns_401(client: TestClient) -> None:
    response = client.get("/conversations")
    assert response.status_code == 401
    assert response.headers.get("www-authenticate") == "Bearer"


def test_proxy_messages_without_jwt_returns_401(client: TestClient) -> None:
    response = client.get("/messages")
    assert response.status_code == 401
    assert response.headers.get("www-authenticate") == "Bearer"


def test_proxy_conversations_with_valid_jwt(
    client: TestClient,
    rsa_pem_pair,
    upstream_client: MagicMock,
) -> None:
    ...
    upstream_client.request.assert_awaited_once()
    call = upstream_client.request.await_args
    assert call.kwargs["url"] == "http://messaging.test/conversations"
    assert call.kwargs["method"] == "GET"


def test_proxy_conversations_nested_path_with_valid_jwt(
    client: TestClient,
    rsa_pem_pair,
    upstream_client: MagicMock,
) -> None:
    ...
    assert response.status_code == 200
    call = upstream_client.request.await_args
    assert call.kwargs["url"] == "http://messaging.test/conversations/direct"
    assert call.kwargs["method"] == "POST"


def test_proxy_messages_with_valid_jwt(
    client: TestClient,
    rsa_pem_pair,
    upstream_client: MagicMock,
) -> None:
    ...
    assert response.status_code == 200
    call = upstream_client.request.await_args
    assert call.kwargs["url"] == "http://messaging.test/messages"
    assert call.kwargs["method"] == "GET"


def test_proxy_messaging_upstream_unavailable(
    client: TestClient,
    rsa_pem_pair,
    upstream_client: MagicMock,
) -> None:
    ...
    upstream_client.request = AsyncMock(
        side_effect=RequestError(
            "boom",
            request=Request("GET", "http://messaging.test/conversations"),
        )
    )
    response = client.get(
        "/conversations",
    ...
    assert response.status_code == 502
    assert "messaging" in response.json()["detail"].lower()
